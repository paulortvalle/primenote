<?php
/**
 * Manipula e Gerencia os dados de usuários
 *
 * Manipula os dados de usuários, faz login e logout,
 * verifica permissões e redireciona a página para
 * usuários logados.
 *
 * @package primenote
 * @since 4.0.1 | 14/02/2017
 */
...
class UserLogin {
	...
	public function check_userlogin () {
		...
		$query = $this->db->query( 
			'SELECT user_id, user, user_session_id, user_password, user_permissions, user_group, user_name, user_short_name FROM users WHERE user = ? LIMIT 1', 
			array( $user ) 
		);
		...
		if ( ! $query ) {
			$this->logged_in = false;
			$this->login_error = 'Não foi possível conectar-se ao banco de dados do sistema';
		...
		if ( empty( $user_id ) ){
			$this->logged_in = false;
			$this->login_error = 'Usuário e senha inválidos';
		...
		if ( $this->phpass->CheckPassword( $user_password, $fetch['user_password'] ) ) {
			...
			if ( session_id() != $fetch['user_session_id'] && ! $post ) { 
				$this->logged_in = false;
				$this->login_error = 'Acesso bloqueado';
			...
			if ( $post ) {
				...
				// user_group definido por 0 = SuperAdmin | 1 = Admin | 2 = Gestores | 3 = Professores | 4 = Alunos
				...
				// Insere um registro de acesso na base de dados
				$log = array();
				$log['login_user_id'] = $user_id;
				$log['login_date'] = date('Y-m-d H:i:s');
				$log['login_remote_addr'] = $_SERVER['REMOTE_ADDR'];
				$log['login_remote_host_addr'] = gethostbyaddr($_SERVER['REMOTE_ADDR']);
				$log['login_remote_port'] = $_SERVER['REMOTE_PORT'];
				$log['login_user_session_id'] = $session_id;
				$loginsert = $this->db->insert('logins', $log);
	...
	protected function goto_login() {
		...
		if ( defined( 'HOME_URI' ) ) {
			...
			// Guarda a página em que o usuário estava
			// $_SESSION['goto_url'] = urlencode( $_SERVER['REQUEST_URI'] );
			$_SESSION['goto_url'] = HOME_URI . '/dashboard/';
